...
%-- including complex cases such as identity verification.
...
%to express applicable rights, notices, records of activities and rights requests, 
...
%information related to rights exercising.
...
%improving the discoverability, management, and preservation of resources, 
...
% DCAT\footnote{\url{http://www.w3.org/ns/dcat} prefixed as \texttt{dcat}}\cite{albertoni_dcat_2024}, 
% DCMI\footnote{\url{http://purl.org/dc/terms/} prefixed as \texttt{dcterms}}~\cite{dcmi_2020}, and 
...
\begin{inlineabc}
    \item[(\textit{\ref{sec:applicable-rights}})] information about the existence of rights and justifications for the exercising of said rights, %related to personal data processing activities,
    %\item[(\textit{\ref{sec:justifications}})] justifications for the exercising of said rights,
...
%, which are omitted from this article due to size restrictions.
...
%This could be the data controller or another entity acting on their behalf, represented using \texttt{dpv:hasDataController} and \texttt{dpv:isImplementedByEntity}, respectively. 
%Additionally, \texttt{foaf:page}\footnote{\url{http://xmlns.com/foaf/0.1/} prefixed as \texttt{foaf}} can be used to reference a document, such as a Webpage, that contains the full details of the notice and offers further functionalities.
...
%, and can be justified using the developed \texttt{Justification} extension.
...
%A \texttt{dcat:CatalogRecord} is also maintained to capture metadata about the record, including details about the data controller and the data subject.
...
%-- from the initial request to its acknowledgement by the data controller, and ultimately to the fulfilment or non-fulfilment of the right.
...
%The \texttt{prov:actedOnBehalfOf} property can also be used to represent delegation or representation, for example, when a parent exercises a right on behalf of a child.
...
%including concepts to justify their fulfilment or non-fulfilment, 
...
%Through these, we hope to enable tools and processes to handle obligations related to rights in an interoperable manner.
...
%and devices that utilise the Web.
...
%More importantly, the development of DPV -- and the rights management concepts presented here -- 
...
%Additionally, the EU Commission has launched an ambitious program\footnote{\url{https://digital-strategy.ec.europa.eu/en/news/eu-digital-identity-4-projects-launched-test-eudi-wallet}} consisting of creating a secure European Digital Identity wallet (EUDI) which will be used by individuals and organisations to assert their identity and access public/private services.
%based on personal data stored in the wallets.
%While work on how to store and manage consent in such wallets based on standards has already been explored~\cite{pandit2024implementing}, the question of how such wallets can facilitate rights exercise has not yet been explored.
...
%This not only facilitates GDPR compliance, but also creates a streamlined experience for users and promotes transparency and accountability.
